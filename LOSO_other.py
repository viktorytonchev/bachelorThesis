...
def annotation_to_category_other(key):
        if key == "Leftover":
            return 0
            ...
            if not flag and not key == "Leftover":
                for values in ground[key]:
                    if start*0.02 >= values[0] and (start + window_size)*0.02 <= values[1]:
                        result_ground.append(annotation_to_category_other(key))
                        flag = True
        if not flag and counter<5:
            result_ground.append(annotation_to_category_other("Leftover"))
...
# This is the function that splits the data into train and test sets. It does so based on given parameters: The set of participants to train on and the participant to test on.
...
def test(train, test, repeats = 5):
    ...
    for matrix in matrices:
        ...
        # Ticket labels
        ax.xaxis.set_ticklabels(['Oth', 'Yog', 'Cer', 'Br', 'Cro', 'Gr', 'Corn', 'Cuc'])
        ax.yaxis.set_ticklabels(['Oth', 'Yog', 'Cer', 'Br', 'Cro', 'Gr', 'Corn', 'Cuc'])
